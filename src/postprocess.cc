...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "../model/knob.txt"
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold,
    ...
    for (int i = 0; i < validCount; ++i) {
        ...
        // 此处非常重要，如果你的目标有10个关键点，则此处j < 10，我的目标只有两个关键点，因此设为j < 2
        for (int j = 0; j < 2; ++j) 
        {
            #ifdef RKNPU1
                ...
                od_results->results[last_count].keypoints[j][0] = ((float)((rknpu2::float16 *)_outputs[3].buf)[j*2 * 8400 + 0 * 8400 + keypoints_index] - letter_box->x_pad)/ letter_box->scale;
                od_results->results[last_count].keypoints[j][1] = ((float)((rknpu2::float16 *)_outputs[3].buf)[j*2 * 8400 + 1 * 8400 + keypoints_index] - letter_box->y_pad)/ letter_box->scale;
                // 如果conf不可用，设置为1.0或忽略
                //od_results->results[last_count].keypoints[j][2] = 1.0f;
            #endif
        }
